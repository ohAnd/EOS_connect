...
class PvAutoscaler:
    ...
    def should_collect(self) -> bool:
        ...
        try:
            ...
            return (datetime.utcnow() - self.last_collection).total_seconds() >= self.collection_interval
    ...
    def collect_if_needed(self) -> bool:
        ...
        # The delta we measure represents generation from the PREVIOUS hour (from last collection to now)
        # So we need to store it as the previous hour's data
        prev_hour_local = (now_boundary.hour - 1) % 24
        prev_date_local = now_boundary.strftime("%Y-%m-%d") if now_boundary.hour > 0 else (now_boundary - timedelta(days=1)).strftime("%Y-%m-%d")
            ...
            if self._pv_interface is not None:
                ...
                if isinstance(arr, list):
                    ...
                    try:
                        ...
                        slots_per_hour = 1
                    # Get forecast for PREVIOUS hour (delta represents generation during that hour)
                    ...
                    if hour_slots:
                        ...
                        logger.info(
                            "[PV-AUTO] Forecast trace: target=%s %02d:00, source=current_pv_forecast, "
                            "resolution=%ss, indices=%d:%d, slots_wh=%s, total_kwh=%.4f",
                            prev_date_local,
                            prev_hour_local,
                            time_frame_base,
                            idx_start,
        ...
        if delta < 0:
            ...
            self._previous_counter_kwh = current_counter
            # Record for PREVIOUS hour (delta represents generation during that hour)
            timeframe_id = (prev_hour_local // 6) + 1
            try:
                self._pv_yield_store.insert_hourly_record(
                    timestamp=utc_period_start.isoformat(),  # Period start timestamp (e.g., 8:00:00 UTC for 8-9 period)
                    ...
                    local_hour=prev_hour_local,  # Timezone-aware local hour (when delta occurred)
                    local_offset_minutes=local_offset_minutes,  # UTC offset for DST tracking
                )
        ...
        try:
            ...
            if latest_db and latest_db.get("timestamp"):
                try:
                    ...
                    # Calculate difference using boundary time for consistent hour counting
                    diff_seconds = (utc_now_boundary.replace(tzinfo=timezone.utc) - last_db_ts).total_seconds()
        ...
        try:
            ...
            if missed_hours <= 1:
                ...
                per_hour = real_delta_kwh / float(missed_hours) if missed_hours > 0 else real_delta_kwh
                ...
                for i in range(1, missed_hours + 1):
                    ...
                    try:
                        local_offset_minutes_i = int(local_ts_i.utcoffset().total_seconds() / 60)
                    except Exception:
                        local_offset_minutes_i = None
                    
                    # For missed hours, use None. For the final hour (most recent), use the forecast
                    # The forecast_kwh fetched earlier is for the previous hour (when delta occurred)
                    if i == missed_hours:
                        # This is the final/current hour - include the forecast
                        forecast_kwh_i = forecast_kwh
                    else:
                        # Historical missed hours - no forecast snapshot available from that time
                        forecast_kwh_i = None

                    real_counter_kwh_i = float(base_counter + per_hour * i)
                    real_delta_kwh_i = float(per_hour)
                    self._pv_yield_store.insert_hourly_record(
                        timestamp=utc_ts_i.isoformat(),  # Keep UTC timestamp for audit
                        ...
                        local_offset_minutes=local_offset_minutes_i,  # UTC offset for DST tracking
                    )
            ...
            try:
                ...
                logger.exception("[PV-AUTO] Failed to compute timeframe scale factors after insert")
        except Exception:
    ...
    def compute_timeframe_scaling_factors(self) -> Dict[int, float]:
        ...
        try:
            ...
            if self._timezone:
                tz = pytz_tz(self._timezone) if isinstance(self._timezone, str) else self._timezone
                today_date = dt.now(tz).date().isoformat()
            else:
                today_date = dt.now().date().isoformat()
        except Exception:
            # Fallback if timezone is not available
            today_date = datetime.now().date().isoformat()

        # Aggregate by (date, timeframe) - EXCLUDING TODAY
        per_day_frame_actual: Dict[str, Dict[int, float]] = defaultdict(lambda: defaultdict(float))
        per_day_frame_forecast: Dict[str, Dict[int, float]] = defaultdict(lambda: defaultdict(float))
        recorded_hours = 0
        ...
        for r in rows:
            ...
            if isinstance(r, dict):
                ...
                # row tuple: id, timestamp, date, hour, timeframe_id, real_counter_kwh, real_delta_kwh, forecast_kwh, created_at
                date = r[2]
                hour = r[3]
