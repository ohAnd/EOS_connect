...
def _card_with(page, summary_extra):
    page.evaluate(
        """(extra) => {
            const load = {id: 'pool', type: 'pool_heatpump', enabled: true,
                reason: 'below target', energy_needed_wh: 20000, planned_wh: 20000,
                plan: [], plan_reasons: [], model: {}, release: null,
                detail: {mean_cop: 4.0},
                plan_summary: Object.assign(
                    {slots: 192, planned: 40, limited_by: null, limited_slots: 0,
                     counts: {}}, extra)};
            document.getElementById('full_screen_content').innerHTML =
                controlsManager._managedLoadCard(load, 900, 0);
        }""",
